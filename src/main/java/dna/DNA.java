...
public class DNA {
     ...
     */
    static {
        NUCLEOTIDE_MASS = Map.of(   'A', 135.128,
                                    'C', 111.103,
                                    'G', 151.128,
                                    'T', 125.107 );

        PROTEIN_START_CODONS = Set.of("ATG");
        PROTEIN_END_CODONS = Set.of("TAA", "TAG", "TGA");

        PROTEIN_NUCLEOTIDES = Set.of('C', 'G');
    }

    private String sequence;
    private Set<String> codons;
    private Map<Character, Double> nucleotideMass = new HashMap<>();
    private double mass; //TODO: remove the static
    ...
    private double computeMass() {
        ...
        for (Character c : sequence.toCharArray()) {
            if (NUCLEOTIDE_MASS.containsKey(c)) {
                ...
                totalMass += JUNK_MASS; // seems like you don't need DNA.JUNK_MASS
            }
        }
    ...
    public DNA cutAndSplice(String restrictionEnzyme, int splicePosition, String splicee) {
         ...
         */
        int i = 0;
        while (i < seqNoJunk.length()) { // does this condition work for multiple restriction enzyme?
            // i < seqNoJunk.length() - restrictionEnzyme.length()
            ...
            if (enzymeMatch) {
                ...
                newSeqBuilder.append(seqNoJunk.substring(i + splicePosition, i + restrictionEnzyme.length())); // this is wrong
    ...
    public boolean equals(Object otherDNA) {
        DNA compareDNA = (DNA) otherDNA;
        return sequence.equals(compareDNA.sequence);
    }
}